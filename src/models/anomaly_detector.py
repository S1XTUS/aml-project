...
class AnomalyDetector:
    ...
    def _extract_time_features(self, df):
        ...
        if 'Timestamp' in df.columns:
            try:
                ...
                if not pd.api.types.is_datetime64_any_dtype(df_time['Timestamp']):
                ...
                logger.warning(f"Could not process Timestamp column: {e}. Dropping it.")
                df_time = df_time.drop(columns=['Timestamp'])
        
        return df_time

    def _create_transaction_features(self, df):
        """Create additional transaction specific features"""
        df_features = df.copy()
    ...
    def preprocess_data(self, df, fit=True):
        ...
        if "Is_Laundering" in df_processed.columns:
            df_processed = df_processed.drop(columns=["Is_Laundering"])
...
# Standalone utility functions (moved outside the class)
def load_data(path):
...
def predict_anomalies_from_model(df_new, model_dir="models"):
    """
...
if __name__ == "__main__":
    ...
    detector_if = train_anomaly_model(
        ...
        contamination=0.05,
        model_dir=os.path.join(MODEL_DIR, "isolation_forest")
    )
    ...
    results = predict_anomalies_from_model(sample_data, os.path.join(MODEL_DIR, "isolation_forest"))
    ...
    print("  results = predict_anomalies_from_model(your_df, 'models')")
